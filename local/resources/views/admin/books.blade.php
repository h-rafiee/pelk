    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="page-header">
                <h1>{!! trans('admin.books') !!}</h1>
                <a href="books/create"><span
                            class="fa fa-file-o"></span> {!! trans('global.create_new',['name'=>trans('admin.book')]) !!}
                </a>
            </div>
        </div>
    </div>
        ...
        <div class="col-sm-12">
                ...
                <table class="table table-bordered">
                    <thead>
                        ...
                        <th>تاریخ انتشار</th>
                        <th>وضعیت</th>
                        <th colspan="2">{!! trans('global.access') !!}</th>
                    </thead>
                    <tbody class="tahoma">
                        <?php $i = 1; ?>
                        @foreach($model as $value)
                            <tr id="rowID{{$value->id}}">
                                ...
                                <td>{{$value->publication->title}}</td>
                                <?php $writerArr=NULL; ?>
                                @foreach($value->writers as $writer)
                                    <?php $writerArr[]=$writer->name; ?>
                                @endforeach
                                <td>{{(empty($writerArr))?trans('global.empty'):implode(" , ",$writerArr)}}</td>
                                <td>{{$value->tags->count()}}</td>
                                <td>{{date("M Y j",strtotime($value->created_at))}}</td>
                                <td>{{($value->active == 1)?'فعال':'غیر فعال'}}</td>
                                <td><a href="{{url("admin/books/{$value->id}/edit")}}" class="text-warning"><span
                                ...
                                <td><a class="remove confirm text-danger" href="javascript.void(0)"
                                       ...
                                       data-link="{{url("admin/books/{$value->id}")}}"><span
