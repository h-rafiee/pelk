    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="page-header">
                ...
                <a href="categories/create"><span
                            class="fa fa-file-o"></span> {!! trans('global.create_new',['name'=>trans('admin.category')]) !!}
                </a>
            </div>
        </div>
    </div>
                ...
                <table class="table table-bordered">
                    ...
                    @foreach($model as $value)
                        <tr id="rowID{{$value->id}}">
                            <td>{{$i++}}</td>
                            <td>{{$value->title}}</td>
                            <td>{{$value->books->count()}}</td>
                            <td>{{date("M Y j",strtotime($value->created_at))}}</td>
                            <td><a href="{{url("admin/categories/{$value->id}/edit")}}" class="text-warning"><span
                            ...
                            <td><a class="remove confirm text-danger" href="javascript.void(0)"
                                   ...
                                   data-link="{{url("admin/categories/{$value->id}")}}"><span
