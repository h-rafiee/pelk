    ...
    <div class="row">
        <div class="col-lg-12">
            ...
            <div class="col-sm-12">
                @if(empty($model))
                    <h2>
                        {!! trans('global.create_new',['name'=>trans('admin.administrator')]) !!}
                    </h2>
                @else
                    <h2>
                        {!! trans('global.edit').' '.trans('admin.administrator') !!} [ {{$model->name}} ]
                    </h2>
                @endif
                ...
                @if(empty($model))
                <form action="{{url('admin/administrators')}}" method="post" class="form">
                @else
                <form action="{{url('admin/administrators/'.$model->id)}}" method="post" class="form">
                    {{ method_field('PUT') }}
                @endif
                {{csrf_field()}}
                ...
                <hr>
                <div class="form-group">
                    <button class="btn btn-primary"><span class="fa fa-save"></span> {!! trans('global.save').' '.trans('admin.administrator') !!}</button>
                </div>
            </form>
            </div>
        </div>
