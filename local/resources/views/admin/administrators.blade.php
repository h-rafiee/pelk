    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="page-header">
                <h1>{!! trans('admin.administrators') !!}</h1>
                <a href="administrators/create"><span
                            class="fa fa-file-o"></span> {!! trans('global.create_new',['name'=>trans('admin.administrator')]) !!}
                </a>
            </div>
        </div>
    </div>
                ...
                <table width="100%" class="table table-striped table-bordered table-hover dataTable">
                    ...
                    @foreach($model as $value)
                        <tr id="rowID{{$value->id}}">
                            ...
                            <td>{{$value->email}}</td>
                            <td>{{(empty($value->mobile))?trans('global.empty'):$value->mobile}}</td>
                            <td>{{date("M Y j",strtotime($value->created_at))}}</td>
                            <td><a href="{{url("admin/administrators/{$value->id}/edit")}}" class="text-warning"><span
                            ...
                            <td><a class="remove confirm text-danger" href="javascript.void(0)"
                                   ...
                                   data-title="{!! trans('admin.confirm_delete',['val'=>$value->name]) !!}"
                                   data-link="{{url("admin/administrators/{$value->id}")}}"><span
